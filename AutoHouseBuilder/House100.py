...
PlayerP = mc.player.getPos()

for House100a in range(10):
    for House100b in range(10):

        class PP:
            x = PlayerP.x + 20 * House100b
            y = PlayerP.y
            z = PlayerP.z + 20 * House100a
            # 領域決定
        regionNS = rn.randint(12, 16)
        regionEW1 = 26 - regionNS
        regionEW2 = rn.randint(-1, 2)
        regionEW = (regionEW1 + regionEW2)

        first_baseX = (PP.x + 1)
        first_baseY = (PP.y - 1)
        first_baseZ = (PP.z + 1)
        end_baseX = (PP.x + regionEW)
        end_baseY = (PP.y - 1)
        end_baseZ = (PP.z + regionNS)
        ...
        mc.setBlocks(first_baseX + 1, first_baseY + 1, first_baseZ + 1, end_baseX - 1, end_baseY + 2, end_baseZ - 1,
                     block.AIR)
        ...
        # 二階の素材
        mc.setBlocks(first_baseX + HbaseA, first_baseY + 1, first_baseZ + HbaseB, end_baseX - HbaseC, end_baseY + 4,
                     end_baseZ - HbaseD, Hmat)
        mc.setBlocks(first_baseX + HbaseA, first_baseY + 5, first_baseZ + HbaseB, end_baseX - HbaseC, end_baseY + 7,
                     end_baseZ - HbaseD, Hmat2)
        ...
        if Hs1 == 7:  # 東西流れ切妻
            ...
            for roofJ_5 in range(2000):
                ...
                if mc.getBlockWithData(randomX, randomY, randomZ) == roofFull:
                    if mc.getBlockWithData(randomX, randomY, randomZ - 1) == block.AIR or mc.getBlockWithData(randomX,
                                                                                                              randomY,
                                                                                                              randomZ + 1) == block.AIR:
        ...
        if Hs1 == 8:  # 南北流れ切妻
            ...
            for roofJ_5 in range(2000):
                ...
                randomY = rn.randint(randomY1, randomY2)
                if mc.getBlockWithData(randomX, randomY, randomZ) == roofFull:
                    if mc.getBlockWithData(randomX - 1, randomY, randomZ) == block.AIR or mc.getBlockWithData(
                            randomX + 1, randomY, randomZ) == block.AIR:
                        if mc.getBlockWithData(randomX, randomY + 1, randomZ) != block.AIR:
                            mc.setBlock(randomX, randomY, randomZ, Hmat2)
        ...
        if os1 == 1:
            ...
            for osE in range(20):
                if NEe_x - osE >= NWe_x:
                    if osE % 2 == 0 or osE == 0:
                        mc.setBlocks(NEe_x - osE, first_baseY + 8 + (osE / 2), NEe_z, SEe_x - osE,
                                     first_baseY + 8 + (osE / 2), SEe_z, roofB)
                        mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2), NEe_z, SEe_x - osE, first_baseY + 7,
                                     NEe_z, Hmat2)
                        mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2), SEe_z, SEe_x - osE, first_baseY + 7,
                                     SEe_z, Hmat2)
                        if NEe_x - osE == NWe_x:
                            mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2), SEe_z, SEe_x - osE, first_baseY + 7,
                                         NEe_z, Hmat2)
                    else:
                        mc.setBlocks(NEe_x - osE, first_baseY + 8 + (osE / 2 - 0.5), NEe_z, SEe_x - osE,
                                     first_baseY + 8 + (osE / 2 - 0.5), SEe_z, roofFull)
                        mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2 - 0.5), NEe_z, SEe_x - osE,
                                     first_baseY + 7, NEe_z, Hmat2)
                        mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2 - 0.5), SEe_z, SEe_x - osE,
                                     first_baseY + 7, SEe_z, Hmat2)
                        if NEe_x - osE == NWe_x:
                            mc.setBlocks(NEe_x - osE, first_baseY + 7 + (osE / 2 - 0.5), SEe_z, SEe_x - osE,
                                         first_baseY + 7, NEe_z, Hmat2)
        ...
        if os1 == 2:
            ...
            for osW in range(20):
                if NWe_x + osW <= NEe_x:
                    if osW % 2 == 0 or osW == 0:
                        mc.setBlocks(NWe_x + osW, first_baseY + 8 + (osW / 2), NWe_z, SWe_x + osW,
                                     first_baseY + 8 + (osW / 2), SWe_z, roofB)
                        mc.setBlocks(NWe_x + osW, first_baseY + 7 + (osW / 2), NWe_z, SWe_x + osW, first_baseY + 7,
                                     NWe_z, Hmat2)
                        mc.setBlocks(SWe_x + osW, first_baseY + 7 + (osW / 2), SWe_z, SWe_x + osW, first_baseY + 7,
                                     SWe_z, Hmat2)
                        if NWe_x + osW == NEe_x:
                            mc.setBlocks(NWe_x + osW, first_baseY + 7 + (osW / 2), NWe_z, SWe_x + osW, first_baseY + 7,
                                         SWe_z, Hmat2)
                    else:
                        mc.setBlocks(NWe_x + osW, first_baseY + 8 + (osW / 2 - 0.5), NWe_z, SWe_x + osW,
                                     first_baseY + 8 + (osW / 2 - 0.5), SWe_z, roofFull)
                        mc.setBlocks(NWe_x + osW, first_baseY + 7 + (osW / 2 - 0.5), NWe_z, SWe_x + osW,
                                     first_baseY + 7, NWe_z, Hmat2)
                        mc.setBlocks(SWe_x + osW, first_baseY + 7 + (osW / 2 - 0.5), SWe_z, SWe_x + osW,
                                     first_baseY + 7, SWe_z, Hmat2)
                        if NWe_x + osW == NEe_x:
                            mc.setBlocks(NWe_x + osW, first_baseY + 7 + (osW / 2 - 0.5), NWe_z, SWe_x + osW,
                                         first_baseY + 7, SWe_z, Hmat2)
        ...
        if os1 == 3:
            ...
            for osS in range(20):
                if SW_z - osS >= NW_z:
                    if osS % 2 == 0 or osS == 0:
                        mc.setBlocks(SEe_x, first_baseY + 8 + (osS / 2), SEe_z - osS, SWe_x,
                                     first_baseY + 8 + (osS / 2), SEe_z - osS, roofB)
                        mc.setBlocks(SEe_x, first_baseY + 7, SEe_z - osS, SEe_x, first_baseY + 7 + (osS / 2),
                                     SEe_z - osS, Hmat2)
                        mc.setBlocks(SWe_x, first_baseY + 7, SEe_z - osS, SWe_x, first_baseY + 7 + (osS / 2),
                                     SEe_z - osS, Hmat2)
                        if SW_z - osS == NW_z:
                            mc.setBlocks(SEe_x, first_baseY + 7, SEe_z - osS, SWe_x, first_baseY + 7 + (osS / 2),
                                         SEe_z - osS, Hmat2)
                    else:
                        mc.setBlocks(SEe_x, first_baseY + 8 + (osS / 2 - 0.5), SEe_z - osS, SWe_x,
                                     first_baseY + 8 + (osS / 2 - 0.5), SEe_z - osS, roofFull)
                        mc.setBlocks(SEe_x, first_baseY + 7, SEe_z - osS, SEe_x, first_baseY + 7 + (osS / 2 - 0.5),
                                     SEe_z - osS, Hmat2)
                        mc.setBlocks(SWe_x, first_baseY + 7, SEe_z - osS, SWe_x, first_baseY + 7 + (osS / 2 - 0.5),
                                     SEe_z - osS, Hmat2)
                        if SW_z - osS == NW_z:
                            mc.setBlocks(SEe_x, first_baseY + 7, SEe_z - osS, SWe_x, first_baseY + 7 + (osS / 2 - 0.5),
                                         SEe_z - osS, Hmat2)
        ...
        if os1 == 4:
            ...
            for osN in range(20):
                if NWe_z + osN <= SWe_z:
                    if osN % 2 == 0 or osN == 0:
                        mc.setBlocks(NWe_x, first_baseY + 8 + (osN / 2), NEe_z + osN, NEe_x,
                                     first_baseY + 8 + (osN / 2), NEe_z + osN, roofB)
                        mc.setBlocks(NWe_x, first_baseY + 7 + (osN / 2), NEe_z + osN, NWe_x, first_baseY + 7,
                                     NEe_z + osN, Hmat2)
                        mc.setBlocks(NEe_x, first_baseY + 7 + (osN / 2), NEe_z + osN, NEe_x, first_baseY + 7,
                                     NEe_z + osN, Hmat2)
                        if NWe_z + osN == SWe_z:
                            mc.setBlocks(NEe_x, first_baseY + 7 + (osN / 2), NEe_z + osN, NWe_x, first_baseY + 7,
                                         NEe_z + osN, Hmat2)
                    else:
                        mc.setBlocks(NWe_x, first_baseY + 8 + (osN / 2 - 0.5), NEe_z + osN, NEe_x,
                                     first_baseY + 8 + (osN / 2 - 0.5), NEe_z + osN, roofFull)
                        mc.setBlocks(NWe_x, first_baseY + 7 + (osN / 2 - 0.5), NEe_z + osN, NWe_x, first_baseY + 7,
                                     NEe_z + osN, Hmat2)
                        mc.setBlocks(NEe_x, first_baseY + 7 + (osN / 2 - 0.5), NEe_z + osN, NEe_x, first_baseY + 7,
                                     NEe_z + osN, Hmat2)
                        if NWe_z + osN == SWe_z:
                            mc.setBlocks(NEe_x, first_baseY + 7 + (osN / 2 - 0.5), NEe_z + osN, NWe_x, first_baseY + 7,
                                         NEe_z + osN, Hmat2)
            ...
            if rainPX == 0:
                ...
                if rainG1 == Hmat2 and rainG3 == Hmat2 and rainG == Hmat2:
                    if mc.getBlock(randomX + rainPX, first_baseY + 3, randomZ + rainPZ) == block.AIR:
                        mc.setBlocks(randomX + rainPX, first_baseY + 1, randomZ + rainPZ, randomX + rainPX,
                                     first_baseY + 6, randomZ + rainPZ, rainB)
        ...
        for ligting in range(10):
            ...
            if lightP == block.AIR:
                if mc.getBlockWithData(randomX, randomY - 1, randomZ) == floorB or mc.getBlockWithData(randomX,
                                                                                                       randomY + 1,
                                                                                                       randomZ) == floorB:
        ...
        if CC3 <= 5:
            ...
            if F2S > 0:
                mc.setBlocks(verandaP1, first_baseY + 4, SW_z + veranda6, verandaP2, first_baseY + 4, SW_z + veranda6,
                             verandaB)
            mc.setBlocks(verandaP1, first_baseY + 5, SW_z + veranda6, verandaP2, first_baseY + 5, SW_z + veranda6,
                         verandaC)
            mc.setBlocks(verandaP1, first_baseY + 6, SW_z + veranda6, verandaP2, first_baseY + 6, SW_z + veranda6,
                         verandaD)
        for berandaW in range(4000):
            randomX = rn.randint(NW_xa, NE_xa)
            randomZ = rn.randint(SW_za - 2, SW_za)
            randomY = rn.randint(int(PP.y + 4), int(PP.y + 5))
            if mc.getBlockWithData(randomX, randomY, randomZ) == verandaC or mc.getBlockWithData(randomX, randomY,
                                                                                                 randomZ) == verandaD:
        ...
        for win1 in range(170):
            ...
            if wnG1 == Hmat:
                if mc.getBlockWithData(randomX - 1, first_baseY + 3, randomZ) == Hmat and mc.getBlockWithData(
                        randomX + 1, first_baseY + 3, randomZ) == Hmat:
                ...
                elif mc.getBlockWithData(randomX, first_baseY + 3, randomZ - 1) == Hmat and mc.getBlockWithData(randomX,
                                                                                                                first_baseY + 3,
                                                                                                                randomZ + 1) == Hmat:
                    winB1 = rn.randint(1, 6)
                    if winB1 <= 3:
        ...
        for win2 in range(250):
            ...
            if wnG1 == Hmat2:
                if mc.getBlockWithData(randomX - 1, first_baseY + 6, randomZ) == Hmat2 and mc.getBlockWithData(
                        randomX + 1, first_baseY + 6, randomZ) == Hmat2:
                ...
                elif mc.getBlockWithData(randomX, first_baseY + 6, randomZ - 1) == Hmat2 and mc.getBlockWithData(
                        randomX, first_baseY + 6, randomZ + 1) == Hmat2:
                    winB1 = rn.randint(1, 5)
                    if winB1 <= 3:
                        winB = 160, 15
                    elif winB1 == 4:
        ...
        for airvent in range(60):
            ...
            if mc.getBlockWithData(randomX, randomY, randomZ) == Hmat or mc.getBlockWithData(randomX, randomY,
                                                                                             randomZ) == Hmat2:
